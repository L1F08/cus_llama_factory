...
#   cls_head 只做一次 forward (无 KV cache 增长)，显存压力小 ——
#   910B 65GB 上 BATCH_SIZE 可以开到 8/12/16，出现 OOM 时下调。
# PREPROCESS_WORKERS: CPU 端视频解码/单样本 processor 的并行线程数。
#   每 rank 处理样本数 < ~1000 时建议开大 (12)，> ~2500 时回落到 4-6。
export BATCH_SIZE=8
export PREPROCESS_WORKERS=12
# 视频参数 (与原 cls_head 单样本脚本一致；不需要可不动)
export INFER_VIDEO_FPS=8.0
export INFER_VIDEO_MAX_PIXELS=602112
...
output_json=/home/ma-user/work/lyf/result1/${taskid_for_sft}/result_0509_cls_head.json #!!!
...
# # # /home/ma-user/work/lyf/test_pullover/data_json/infer_converted_data.json #!!! **修改为你的推理数据路径**
# output_json=/home/ma-user/work/lyf/result1/${taskid_for_sft}/result_0430_cls_head.json #!!!
#!!!
####################### infer task #######################
echo "=================================================="
echo "  cls_head batch inference — forward + 分类头"
...
# 模型训练参数
# master_port 与其它脚本错开 (qwen35=6662, logits_nat=6663, cls_head=6664)
DISTRIBUTED_ARGS="
    ...
    --master_port 6664
"
torchrun $DISTRIBUTED_ARGS /home/ma-user/work/lyf/infer_faster_qwen35_cls_head_batch.py \
