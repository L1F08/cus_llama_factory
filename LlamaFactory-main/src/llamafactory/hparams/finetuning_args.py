...
class FinetuningArguments(
    ...
    # ===== Binary classification head (generic) =====
    # Replaces token-level CE loss with a small MLP head on hidden state.
    # Works for any binary task: define positive/negative class via two answer texts.
    use_cls_head: bool = field(
        default=False,
        metadata={"help": "Use binary classification head on hidden state instead of LM CE loss."},
    )
    cls_head_dropout: float = field(
        default=0.1,
        metadata={"help": "Dropout in classification head."},
    )
    cls_focal_alpha: float = field(
        default=0.5,
        metadata={
            "help": (
                "Focal loss alpha (weight for the positive class, i.e. cls_positive_text). "
                ">0.5 prefers recall on the positive class; <0.5 prefers recall on the negative class."
            )
        },
    )
    cls_focal_gamma: float = field(
        default=2.0,
        metadata={"help": "Focal loss gamma (focusing parameter)."},
    )
    cls_positive_text: str = field(
        default="高风险",
        metadata={
            "help": (
                "Answer text for the POSITIVE class (label=1). "
                "Its first tokenized token id is used to identify positive samples."
            )
        },
    )
    cls_negative_text: str = field(
        default="安全",
        metadata={
            "help": (
                "Answer text for the NEGATIVE class (label=0). "
                "Its first tokenized token id is used to identify negative samples."
            )
        },
    )
    cls_lm_loss_weight: float = field(
        default=0.0,
        metadata={"help": "If >0, blend in original LM CE loss with this weight (regularization)."},
    )
